        ...
        <div className="max-w-2xl mx-auto px-4 pt-10 pb-16 space-y-6">
          ...
          <div className="animate-in text-center space-y-1 pt-4">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur text-white/80 text-xs font-semibold px-3 py-1 rounded-full border border-white/20 mb-3">
              ...
              {data.is_fallback ? "Harga Rujukan / Reference Price" : "Langsung dari KPDNHEP / Live"}
            </div>
            <h1 className="text-4xl font-black text-white drop-shadow-lg">⛽ Harga Petrol Malaysia</h1>
            <p className="text-white/60 text-sm">Malaysia Petrol Price · Dikemas kini setiap Khamis / Updated every Thursday</p>
          </div>
          ...
          <div className="animate-in delay-1 card-glass rounded-2xl px-5 py-3 flex flex-wrap items-center justify-between gap-2 text-sm">
            <div>
              <div className="text-white/40 text-xs uppercase tracking-wider">Berkuat kuasa / Effective</div>
              <div className="text-white font-semibold">{data.effective_date}</div>
            </div>
            <div className="text-right">
              <div className="text-white/40 text-xs uppercase tracking-wider">Kemaskini seterusnya / Next update</div>
            ...
            {data.fuels.map((fuel, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Market price (strikethrough) for subsidised fuels */}
                  {fuel.market_price && (
                    <div className="text-sm text-white/40 line-through mb-0.5">
                      RM {fuel.market_price.toFixed(2)}{" "}
                      <span className="no-underline text-xs">(tanpa subsidi / without subsidy)</span>
                    </div>
                  )}
                  ...
                  <div className="text-xs text-white/40 mt-0.5">se-liter / per litre · {fuel.note_ms}</div>
                  ...
                  {saving && saving > 0 && (
                    <div className={`mt-3 inline-flex items-center gap-1 text-xs text-white font-semibold px-2.5 py-1 rounded-full ${style.saveBadge}`}>
                      💰 Jimat / Save RM {saving.toFixed(2)}/L
                    </div>
                  )}
                </div>
              );
      ...
      <div className="max-w-2xl mx-auto px-4 pb-10 space-y-6 bg-[#0a0a0a]">
        ...
        <div className="grid sm:grid-cols-2 gap-4 animate-in delay-3">
          {[
            {
              icon: "📅",
              title: "Bila Harga Berubah? / When Do Prices Change?",
              body: "Dikemas kini setiap Khamis tengah malam di bawah Mekanisme Harga Automatik (APM).\n\nUpdated every Thursday midnight under Malaysia's Automatic Pricing Mechanism (APM).",
            },
            {
              icon: "🟡",
              title: "RON95 — Bersubsidi / Subsidised",
              body: "RON95 ialah petrol paling biasa di Malaysia dengan harga siling kerajaan. Sesuai untuk kebanyakan kereta penumpang.\n\nMost common petrol in Malaysia with a government ceiling price. Suitable for most passenger cars.",
            },
            {
              icon: "🟢",
              title: "RON97 — Harga Pasaran / Market Price",
              body: "RON97 ialah petrol premium tanpa subsidi mengikut harga pasaran. Disyorkan untuk enjin turbo dan berprestasi tinggi.\n\nPremium unsubsidised petrol. Recommended for turbocharged and high-performance engines.",
            },
            {
              icon: "🔵",
              title: "Diesel Euro 5 — Harga Pasaran / Market Price",
              body: "Diesel Euro 5 ialah diesel bersih tanpa subsidi. Diesel B10 mungkin bersubsidi untuk kenderaan komersial dan pertanian yang layak.\n\nClean unsubsidised diesel. B10 may be subsidised for eligible commercial and agricultural vehicles.",
            },
          ].map((item) => (
            <div key={item.title} className="card-glass rounded-2xl p-5 hover:bg-white/10 transition-colors">
              ...
              <p className="text-xs text-white/50 leading-relaxed whitespace-pre-line">{item.body}</p>
            </div>
          ))}
        </div>
        ...
        <div className="text-center text-xs text-white/25 space-y-1 animate-in delay-4">
          ...
          <p>Harga untuk rujukan sahaja · Prices are for informational reference only</p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 text-sm pb-4">
          ...
          <Link href="/privacy-policy" className="text-yellow-400 hover:underline">Dasar Privasi / Privacy</Link>
          <span className="text-white/20">·</span>
          <Link href="/terms" className="text-yellow-400 hover:underline">Terma / Terms</Link>
        </div>
      </div>
    </div>
  );
